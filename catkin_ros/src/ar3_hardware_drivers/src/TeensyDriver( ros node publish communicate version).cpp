...
TeensyDriver::TeensyDriver(ros::NodeHandle& nh)
: serial_port_(io_service_), n(nh) 
{
	teensyDriver_pub = n.advertise<std_msgs::String>("teensyDriver", 1000);
	sub = nh.subscribe("teensyResponse", 1000, teensyCallback );
	whetherCallback = false;
...
void TeensyDriver::update(std::vector<double>& pos_commands, std::vector<double>& joint_positions)
{
    ...
    outMsg += "\n";
    //jcheng2020 revised
    std::string report = "Submitted Message to Teensy: " + outMsg;
...
void TeensyDriver::exchange(std::string outMsg)
{
    ...
    {
        ROS_INFO("transmit(outMsg, errTransmit) err");// print err
    }

    if (!receive(inMsg, errReceive))
    {
        ROS_INFO("receive(inMsg, errReceive) err");// print err
    }
    // parse msg
    ...
    {
    	//jcheng2020 revised
    	std::string report = "Received EncSteps Message from Teensy: " + inMsg;
    	ROS_INFO(report.c_str());
    	//jcheng2020 revised end
        // encoder steps
...
bool TeensyDriver::transmit(std::string msg, std::string& err)
{
    ...
    teensyDriver_pub.publish(buffer);
    ROS_INFO("%s\n",buffer.data.c_str());
    ros::spinOnce();
    if(ros::ok() == true)
    {
...
void TeensyDriver::encStepsToJointPos(std::vector<int>& enc_steps, std::vector<double>& joint_positions)
{
    ...
    {
        //jcheng2020 revised for J5 rotation reverse
        ...
        if ( i == 4)
        {
        	joint_positions[i] = (-1.0) * static_cast<double>(enc_steps[i]) / enc_steps_per_deg_[i];
        }
        //jcheng2020 revised end
    }
}
...
void TeensyDriver::jointPosToEncSteps(std::vector<double>& joint_positions, std::vector<int>& enc_steps)
{
    for (int i = 0; i < joint_positions.size(); ++i)
    {
    	...
    	if ( i == 4)
    	{
     		enc_steps[i] = (-1.0) * static_cast<int>(joint_positions[i] * enc_steps_per_deg_[i]);
    	}
    	//jcheng2020 revised end
    }
}

void TeensyDriver::teensyCallback(const std_msgs::String::ConstPtr& msg)
{
	buffer = std::string(msg->data.c_str());
	while(true)
	{
	ROS_INFO("msgs received" );
	
	}
	whetherCallback = true;
}
