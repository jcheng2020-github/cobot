//<description>MoveIt script for chess_player_motion_series test MOVEIT P&P SOLUTION TEST move_group_demo( with grasp msgs for coordinate record) </description>
...
const double pieceDiameter = 0.030;
std::string result;
...
const double piecePositionZ = 0.10;
...
void addCollisionChessboard(std::vector<moveit_msgs::CollisionObject>& collision_objects, moveit::planning_interface::PlanningSceneInterface& planning_scene_interface)
{
	...
	if(whetherRepeat == false)
	{
		...
		collision_objects[object_number].primitives[0].dimensions[2] = 0.055;
		...
		collision_objects[object_number].primitive_poses[0].position.z = 0.0275;
...
void addCollisionPiece(std::string name, double x, double y, double z, std::vector<moveit_msgs::CollisionObject>& collision_objects, moveit::planning_interface::PlanningSceneInterface& planning_scene_interface)
{
	...
	for(int i= 0; i < object_number; i++)
	{
		if(collision_objects[i].id == name)
		{
			whetherRepeat = true;
		}
	}
	...
	if(whetherRepeat == false)
	{
		...
		collision_objects.resize(object_number + 1);
		collision_objects[object_number].id = name;
		  collision_objects[object_number].header.frame_id = "base_link";
		  ...
		  collision_objects[object_number].primitives[0].type = collision_objects[object_number].primitives[0].BOX;
		  collision_objects[object_number].primitives[0].dimensions.resize(3);
		  collision_objects[object_number].primitives[0].dimensions[0] = 0.017;
		  collision_objects[object_number].primitives[0].dimensions[1] = 0.017;
		  collision_objects[object_number].primitives[0].dimensions[2] = 0.086;
		  */
		...
		collision_objects[object_number].primitives[0].dimensions[primitive.CYLINDER_HEIGHT] = 0.095;
		collision_objects[object_number].primitives[0].dimensions[primitive.CYLINDER_RADIUS] = pieceDiameter / 2;
		  ...
		  collision_objects[object_number].primitive_poses[0].position.z = z + 0.003/*+0.005*/;
		  collision_objects[object_number].primitive_poses[0].orientation.w = 1.0;
...
bool aim(double x, double y, double z, GripperInstruction clawInstruction)
{
...
void aimWithCheck(double x, double y, double z, GripperInstruction clawInstruction)
{
	moveit::planning_interface::MoveGroupInterface group("ar3");
	group.setPlanningTime(2.0);
	bool success = false;
	success = aim(x, y, z  + riseDistanceForPieceHeight, clawInstruction);//pursue retrieve distance 0.110
	
	
	if(success == false)
	{
		ROS_INFO("backup aim");
		aim(x, y, z  + minRiseDistance, clawInstruction);
	}
	else
	{
...
void uci2tfPickPlace(std::string uci)
{
	...
	//transit
	aimWithCheck(object_x, object_y, object_z, Open);//pursue retrieve distance 0.110
	...
	group2.attachObject("target", "ar3_hand");
	//pick
	ros::Duration( 3.0).sleep();
	aimWithCheck(object_x, object_y, object_z, Close);
	
	//transit
	test_constraints.orientation_constraints.resize(0);
	aimWithCheck(end_x, end_y, end_z, Close);
	...
	group2.detachObject("target");
	//place
	ros::Duration( 3.0).sleep();
	aimWithCheck(end_x, end_y,  end_z, Open);
...
void manipulate(std::string instruction)
{
	...
	group2.setJointValueTarget(joint_group_positions);
	group2.move();
	//double x_cooridnates[9] = {tfXOrigin_begin ,
	...
	//tfXOrigin_begin - 0.047 - 0.043 - 0.046 -0.047 -0.046 - 0.046 -0.046 - 0.046};
	
	//double Y_cooridnates[8] = {tfYOrigin_begin,
	...
	//double coordinates[8][8][2]
	//={{},{},{},{},{},{},{},{}} ;
	
	//aimWithCheck(tfXOrigin_begin, tfYOrigin_begin - 0.044 - 0.044 - 0.044 - 0.038 - 0.038 - 0.047 - 0.041, 0.260, Open);
	
	//group.setPoseTarget(target_pose1);
	//group.move();
	  
	  
//	aimWithCheck(tfXOrigin_begin  -0.047 -0.047 - 0.047 - 0.041 -0.050 -0.047/* - 0.047*/
//	,tfYOrigin_begin +0.010/*- 0.044*/
//	,0.260, Open);
