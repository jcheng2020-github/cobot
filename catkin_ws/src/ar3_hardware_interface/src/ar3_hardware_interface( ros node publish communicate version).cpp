...
//with TeensyDriver(( ros node publish communicate version))  
#include <sstream>
...
AR3HardwareInterface::AR3HardwareInterface(ros::NodeHandle &nh) : nh_(nh), driver_(nh)
{
	sub = nh.subscribe("deviation_msgs", 1000, deviation_callback);
	...
	for (int i = 0; i < num_joints_; ++i)
	{
		...
		joint_positions_[i] = degToRad(actuator_positions_[i] + joint_offsets_[i] + joint_deviation_adjustment[i]);
...
void AR3HardwareInterface::init()
{
	...
	for(int i = 0; i < 6 ; i++)
	{
		joint_deviation_adjustment[i] = 0.0;
	}
...
void AR3HardwareInterface::write(ros::Duration elapsed_time)
{
	for (int i = 0; i < num_joints_; ++i)
	{
		...
		actuator_commands_[i] = radToDeg(joint_position_commands_[i]) - joint_offsets_[i] - joint_deviation_adjustment[i];
	}
}
...
void AR3HardwareInterface::deviation_callback(const std_msgs::String::ConstPtr& msg)
{
	std::string media = "";
	bool permission = true;
	ROS_INFO("I heard: [%s]", msg->data.c_str());
	for(int i = 0; i < strlen(msg->data.c_str()) ; i++)
	{
		media += msg->data.c_str()[i];
	}
	for(int i = 0 ; i < 6; i ++)
	{
		char letter = 'A' + i;
		std::string reference = "";
		reference.push_back(letter);
		if(media.find(reference) == std::string::npos)
		{
			permission = false;
		}
	}
	if(permission == true)
	{
		...
		{
			char letter = 'A' + i;
			std::string referenceBegin = "";
			std::string referenceEnd = "";
			size_t indexBegin;
			size_t indexEnd;
			
			referenceBegin.push_back(letter);
			indexBegin = media.find(referenceBegin) + 1;
			if(i < 5)
			{
				referenceEnd.push_back(char(letter + 1));
				indexEnd = media.find(referenceEnd);
			}
			else
			{
				indexEnd = media.length();
			}
			
			joint_deviation_adjustment[i] = std::stod(media.substr(indexBegin, indexEnd-indexBegin));
			ROS_INFO("%f",joint_deviation_adjustment[i]);
		}
	}
}
